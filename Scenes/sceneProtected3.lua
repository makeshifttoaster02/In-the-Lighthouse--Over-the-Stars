SceneProtected3 = Hidable:extend()

function SceneProtected3:new()
    self.entries = {
        SceneProtected3Entry1()
    }
...
function SceneProtected3:update(dt, cursorX, cursorY)
...
function SceneProtected3:mousereleased(cursorX, cursorY)
    if self.visible then
        for i = 1, #self.entries do
            local currEntry = self.entries[i]
            if self:withinBox(cursorX, cursorY) then
...
function SceneProtected3:wheelmoved(y)
    if self.visible then
        -- if y > 0 then
...
function SceneProtected3:keypressed(key)
    if self.visible then
...
function SceneProtected3:getScrollOffset()
    return self.scrollOffset
end

function SceneProtected3:resetScrollOffset()
    self.scrollOffset = 0
end

function SceneProtected3:restoreDefaults()
    -- self.scrollOffset = 0
end

function SceneProtected3:withinBox(cursorX, cursorY)
